...
def firestore_write(
    path: str, fields: dict, token: str, create_only: bool = False
) -> dict:
    """PATCH creates or replaces a document at ``path``.

    With ``create_only``, an existing document is left alone and the result
    has ``__error`` 409 (or 400, depending on how Firestore phrases it).
    """
    url = (
...
#
# They start with no likes and no comments. Those counts are only ever what
# people actually did — the rules move them one at a time, beside the like or
# comment itself. Seeding made-up numbers here once left posts saying "13
# comments" over a thread of one.

POSTS = [
    ("rifat", "EUR/USD", -1.0, 2,
     "H4 ডিমান্ড জোনে বুলিশ পিন বার, লন্ডন ওপেনের আগে এন্ট্রি।",
     "স্টপ লেগেছে। কিন্তু সেটআপ, সাইজ, স্টপ — সব প্ল্যান মতো ছিল। এরকম ১০টা "
     "ট্রেডের ৪টা হারলেও সমস্যা নেই। আজ কিছু বদলাব না।", True),
    ("imran", "GBP/USD", 4.8, 6,
     "নিউজের আগে ঢুকেছিলাম, বড় মুভ ধরব ভেবে।",
     "৪.৮R পেয়েছি, কিন্তু রিস্ক ছিল ৭%। উল্টো দিকে গেলে অ্যাকাউন্টের "
     "এক-তৃতীয়াংশ চলে যেত। এটা ভালো ট্রেড ছিল না, ভাগ্য ভালো ছিল।", False),
    ("nusrat", "USD/JPY", 2.1, 11,
     ...
     "ওই অপেক্ষাটাই স্টপ ২৫ পিপ থেকে ১২ পিপে নামিয়ে দিয়েছে।", True),
    ("tasnim", "EUR/USD", 2.4, 4,
     ...
     "না-ট্রেড করাটাও একটা সিদ্ধান্ত, এটা বুঝতে ছয় মাস লেগেছে।", True),
    ("ishrat", "GBP/USD", -1.0, 9,
     ...
     "হারার সিরিজে সবচেয়ে কঠিন কাজ হলো কিছু না বদলানো।", True),
    ("jarin", "EUR/USD", -1.0, 26,
     ...
     "স্টপ ৩৫ পিপ দূরে ছিল, তাই লট আরও ছোট হওয়া উচিত ছিল।", True),
    ...
    ("farhana", "GBP/USD", 3.0, 120,
     ...
     "অর্ডার ছুঁইনি। এই একটা ট্রেড অনেক কিছু বদলে দিল।", True),
    ("ayan", "EUR/USD", 1.9, 1,
     "ডেইলি সাপোর্টে তৃতীয়বার টাচ, প্রতিবারই উপরে ফিরেছে।",
     "তৃতীয় টাচে ঢুকেছি, প্রথমটায় না। প্রথম টাচ একটা অনুমান, তৃতীয়টা "
     "একটা প্যাটার্ন। অপেক্ষাটা বিরক্তিকর, কিন্তু ওটাই কাজের।", True),
    ("tanvir", "GBP/USD", -1.0, 3,
     "H4 রেজিস্ট্যান্সে বেয়ারিশ এনগাল্ফিং, ভলিউম ঠিক ছিল।",
     "স্টপ লেগেছে ২ পিপের জন্য, তারপর আমার দিকেই গেছে। রাগ লাগছিল, কিন্তু "
     "স্টপ আরও দূরে দিলে অন্য দশটা ট্রেডে বেশি হারতাম। স্টপ ঠিক ছিল।", True),
    ("sadia", "USD/JPY", 2.8, 5,
     "এশিয়ান সেশনের লো ভেঙে আবার উপরে, লিকুইডিটি সুইপ।",
     "প্রথমবার সুইপ চিনতে পেরেছি। আগে এটাকেই ব্রেকডাউন ভেবে উল্টো দিকে "
     "ঢুকতাম। একটা কনসেপ্ট বুঝতে আমার চার মাস লেগেছে।", True),
    ("shakib", "EUR/USD", -1.0, 7,
     "লন্ডন ওপেনে মোমেন্টাম ধরার চেষ্টা।",
     "আজ পাঁচটা ট্রেড নিয়েছি। নিয়ম তিনটা। বাকি দুইটা বিরক্তি থেকে নেওয়া, "
     "সেটআপ থেকে না। লস ছোট, কিন্তু অভ্যাসটা বড় সমস্যা।", False),
    ("jarin", "GBP/USD", 2.2, 8,
     "উইকলি ডিমান্ড জোন, ডেইলিতে বুলিশ ক্লোজ।",
     "এই সপ্তাহে দুইটা ট্রেড। দুইটাই প্ল্যান মতো। বেশি ট্রেড করলে বেশি "
     "শিখি না — বেশি ভুল করি।", True),
    ("mim", "EUR/USD", 0.9, 10,
     "১৫ মিনিটে ডাবল বটম, ছোট স্টপ।",
     "প্রথম লাভের ট্রেড! হাত কাঁপছিল, তাই তাড়াতাড়ি বের হয়েছি। টার্গেট "
     "ছিল ২R, পেয়েছি ০.৯R। ভয় ছাড়তে সময় লাগবে।", True),
    ("anika", "USD/JPY", -1.0, 13,
     "ট্রেন্ডলাইন বাউন্স, H1 কনফার্মেশন।",
     "হেরেছি, কিন্তু আজকের রিস্ক ছিল ০.৮%। গত মাসে এই একই ট্রেডে ৩% "
     "রিস্ক নিতাম। ফলাফল একই, ক্ষতিটা এক-চতুর্থাংশ।", True),
    ("proma", "GBP/USD", 3.4, 16,
     "নিউজের পরে সেটেল হওয়ার অপেক্ষা করেছি, তারপর রিটেস্টে।",
     "নিউজের সময় ঢুকিনি — ৪০ মিনিট অপেক্ষা করেছি। আগে নিউজেই ঝাঁপাতাম আর "
     "স্প্রেড তিন গুণ হয়ে যেত। অপেক্ষাটাই এজ।", True),
    ("sabbir", "EUR/USD", -2.0, 19,
     "মনে হলো নিচে যাবে, শর্ট দিয়েছি।",
     "\'মনে হলো\' দিয়ে শুরু হওয়া প্রতিটা ট্রেড আমি হেরেছি। জার্নাল "
     "খুলে গুনলাম — সাতটার সাতটাই। এটা আর লিখব না।", False),
    ("nusrat", "EUR/USD", 1.6, 22,
     "ডেইলি রেঞ্জের নিচের সীমা, ছোট স্টপ দিয়ে।",
     "টার্গেটের আগে বের হয়েছি কারণ সেশন শেষ হচ্ছিল। ঘুমের সময় পজিশন "
     "রাখি না — এটা নিয়ম, লাভ কমলেও।", True),
    ("rifat", "USD/JPY", 2.0, 28,
     "সাপ্তাহিক লেভেল, চার ঘণ্টার চার্টে কনফার্মেশন।",
     "টানা চারটা হারার পরে এটা। কিছু বদলাইনি বলেই এটা এসেছে। সিস্টেম "
     "বদলালে আজ আমি অন্য কিছু করতাম আর এটা মিস করতাম।", True),
    ("imran", "EUR/USD", -4.2, 31,
     "আগের ট্রেডে হেরে সাইজ দ্বিগুণ করেছি, ফেরত আনতে।",
     "ডাবল করে হেরেছি। এখন ফেরত আনতে তিনটা জেতা ট্রেড লাগবে। এটাই সেই "
     "ফাঁদ যেটার কথা সবাই বলে আর কেউ শোনে না।", False),
    ("tasnim", "GBP/USD", 1.2, 36,
     "রেঞ্জের মাঝখান থেকে উপরের সীমায়।",
     "ছোট লাভ, কিন্তু আজ আমার নিয়ম ছিল সর্বোচ্চ দুইটা ট্রেড। দুইটাই "
     "নিয়েছি, তারপর অ্যাপ বন্ধ করেছি।", True),
    ("raihan", "USD/JPY", -1.0, 44,
     "ব্রেকআউটের রিটেস্ট, স্ট্যান্ডার্ড সেটআপ।",
     "হারলাম। এক্সপেক্টেন্সি দেখলাম — এখনো +০.৩R প্রতি ট্রেডে। একটা "
     "ট্রেড কিছু বলে না, একশোটা বলে।", True),
    ("arif", "EUR/USD", 2.6, 51,
     "ডেইলি ট্রেন্ডের দিকে H1 পুলব্যাক।",
     "ট্রেন্ডের বিপরীতে ট্রেড করা বন্ধ করেছি এক মাস হলো। উইন রেট ৪১% "
     "থেকে ৫৫% এ উঠেছে। একটাই নিয়ম বদলেছি।", True),
    ("ishrat", "EUR/USD", 2.9, 58,
     "উইকলি সাপোর্ট, ডেইলিতে পিন বার।",
     "এই মাসে আমার সেরা ট্রেড, আর আমি কিছুই বিশেষ করিনি। শুধু "
     "সেটআপটার জন্য ছয় দিন অপেক্ষা করেছি।", True),
    ("mehedi", "GBP/USD", 1.1, 66,
     "সাপোর্টে বাউন্স, ছোট সাইজ।",
     "আজ প্রথমবার স্টপ সরাইনি। জিতেছি কিনা সেটা বড় কথা না — আজ আমি "
     "নিজের কথা রেখেছি।", True),
    ("niloy", "EUR/USD", -1.0, 79,
     "ডেইলি লেভেল, রিটেস্টে এন্ট্রি।",
     "হেরেছি এবং কারণটা লিখে রাখছি: সেশন শুরুর আগে ঢুকেছিলাম, ভলিউম "
     "ছিল না। ভলিউম ছাড়া লেভেল শুধু একটা দাগ।", True),
    ("sadia", "GBP/USD", -1.0, 91,
     "রেঞ্জ ব্রেক ধরার চেষ্টা।",
     "ফলস ব্রেক। এখন বুঝি ব্রেকআউটের ৭০% ফলস হয় — তাই রিটেস্টের "
     "অপেক্ষা করা লাভ কমায় না, বাঁচায়।", True),
    ("ayan", "USD/JPY", 2.3, 104,
     "ডেইলি ট্রেন্ড, চার ঘণ্টায় পুলব্যাক শেষ।",
     "এই সপ্তাহে তিনটা ট্রেড, তিনটাই একই সেটআপ। বিরক্তিকর শোনায়, কিন্তু "
     "একটা সেটআপ ভালো করে শেখা দশটা মাঝারি জানার চেয়ে ভালো।", True),
    ("niloy", "USD/JPY", 0.3, 147,
     ...
     "কার্যত ব্রেক-ইভেন — আর একটা নষ্ট সেটআপ।", False),
]
...
def seed_posts(token: str, uids: dict) -> None:
    ...
    for index, (author, symbol, r, hours_ago, reason, lesson,
                followed) in enumerate(POSTS):
        uid = uids.get(author)
        if uid is None:
        ...
        result = firestore_write(
            f"posts/seed-{index}",
            {
                ...
                # Nothing has happened to it yet. See the note above POSTS.
                "claps": i(0),
                "commentCount": i(0),
                # Must be present, not merely defaulted on read: Firestore's
                # orderBy skips documents that lack the field entirely, so a
                # post with no reach is a post the feed cannot see.
                "reach": i(0),
            ...
            token,
            # Re-running must not wipe what people have done to a post since.
            create_only=True,
        )
        if result.get("__error") in (400, 409) and "exists" in result.get("body", ""):
            print(f"  · post {index} already there, left as it is")
