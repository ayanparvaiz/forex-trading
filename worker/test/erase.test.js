...
/** The same calls as restStore, over a Map of path → fields. */
function memoryStore(docs, { budget = Infinity } = {}) {
  let spent = 0;
  const spend = () => {
    ...
    async find({ collection, group = false, field, op = '==', value, limit }) {
      ...
      return sorted()
        ...
        .filter((p) => {
          ...
          return op === 'array-contains' ? Array.isArray(v) && v.includes(value) : v === value;
        })
        .slice(0, limit)
        .map((p) => ({ path: p, data: {} }));
    },
